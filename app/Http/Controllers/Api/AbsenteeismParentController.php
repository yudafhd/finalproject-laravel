...
class AbsenteeismParentController extends Controller
{
    ...
    public function submitAbsent(Request $request)
    {
        try {
            ...
            if (is_array(json_decode($request->date))) {
                foreach (json_decode($request->date) as $key => $value) {
                    $day = $this->switchDayName(date('D', $value));
                    ...
                    $userScheduleToday = Schedule::where('kelas_id', $user->kelas_id)
                        ->where('day', $day)
                        ->get();
